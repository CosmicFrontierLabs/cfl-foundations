...
parser.add_argument('--no-sinusoid', action='store_true', help='Disable sinusoid fitting and residual plots')
...
# Conditionally fit sinusoids to x and y data
if not args.no_sinusoid:
...
if not args.no_sinusoid:
    ...
    axes[0, 0].axhline(y=x_positions.mean(), color='k', linestyle=':', linewidth=1, alpha=0.7, label=f'Mean: {x_positions.mean():.2f}')
...
if not args.no_sinusoid:
    ...
    axes[0, 1].axhline(y=y_positions.mean(), color='k', linestyle=':', linewidth=1, alpha=0.7, label=f'Mean: {y_positions.mean():.2f}')
...
# Plot 5: X vs Y (trajectory)
traj_row = 2 if not args.no_sinusoid else 1
axes[traj_row, 0].scatter(x_positions, y_positions, c=timestamps_rel, cmap='viridis', s=2, alpha=0.4)
if not args.no_sinusoid:
    ...
    axes[traj_row, 0].plot(x_circle, y_circle, 'r--', linewidth=0.5, alpha=0.9, label='Nominal circle')
    axes[traj_row, 0].plot(x_params[3], y_params[3], 'r+', markersize=15, markeredgewidth=3, label='Center')
    axes[traj_row, 0].legend(loc='best', fontsize=8)
axes[traj_row, 0].set_xlabel('X Position (pixels)')
axes[traj_row, 0].set_ylabel('Y Position (pixels)')
axes[traj_row, 0].set_title('Tracking Trajectory (colored by time)')
axes[traj_row, 0].grid(True, alpha=0.3)
axes[traj_row, 0].set_aspect('equal', adjustable='box')
cbar = plt.colorbar(axes[traj_row, 0].collections[0], ax=axes[traj_row, 0])
cbar.set_label('Time (s)')

# Plot 6: Timestamp jitter histogram
jitter_row = 2 if not args.no_sinusoid else 1
axes[jitter_row, 1].hist(dt * 1000, bins=50, alpha=0.7, edgecolor='black')
axes[jitter_row, 1].set_xlabel('Time between updates (ms)')
axes[jitter_row, 1].set_ylabel('Count')
axes[jitter_row, 1].set_title(f'Timestamp Jitter\nMean: {np.mean(dt)*1000:.2f}ms, Std: {np.std(dt)*1000:.2f}ms')
axes[jitter_row, 1].grid(True, alpha=0.3)
