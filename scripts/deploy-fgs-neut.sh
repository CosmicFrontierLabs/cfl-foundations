...
# Deploy fgs_server to Neutralino (NSV455 camera)
#
# Usage:
#   ./deploy-fgs-neut.sh           # Update: build, sync, restart service
#   ./deploy-fgs-neut.sh --setup   # Full setup: build, sync, install service, enable on boot

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"

# Configuration
REMOTE_HOST="cosmicfrontiers@orin-416.tail944341.ts.net"
...
usage() {
    ...
    echo ""
    echo "Deploy fgs_server (NSV455 camera) to Neutralino."
    echo ""
    echo "Modes:"
...
# Step 1: Build fgs_server on Neutralino
print_info "Building fgs_server on Neutralino..."
"$SCRIPT_DIR/build-remote.sh" --package test-bench --binary fgs_server --neut
...
print_info "Syncing frontend files to Neutralino..."
rsync -avz \
...
[Unit]
Description=FGS Camera Server (NSV455)
After=network.target

[Service]
Type=simple
User=cosmicfrontiers
WorkingDirectory=/home/cosmicfrontiers/rust-builds/meter-sim
Environment=RUST_LOG=info
ExecStart=/home/cosmicfrontiers/rust-builds/meter-sim/target/release/fgs_server --camera-type nsv
Restart=on-failure
RestartSec=5
...
if [ "$SETUP_MODE" = true ]; then
    ...
    print_info "Port 80 redirect enabled: access via http://neutralino/"
fi
print_info ""
