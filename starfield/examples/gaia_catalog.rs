...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, (min, max, _)) in mag_ranges.iter().enumerate() {
        ...
        let stars_in_range = catalog.filter(|star| star.phot_g_mean_mag >= *min && star.phot_g_mean_mag < *max).len();
    ...
    let variable_stars = catalog.filter(|star| star.phot_variable_flag == "VARIABLE").len();
    ...
    let mut stars_vec: Vec<_> = catalog.stars()
    ...
    for (i, star) in stars_vec.iter().take(5).enumerate() {
        ...
        println!("  {}. Source ID {} - Proper Motion: {:.2} mas/year", 
                i + 1, star.source_id, pm);
    }
