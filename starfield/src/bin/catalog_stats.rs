...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, (min, max, _)) in mag_ranges.iter().enumerate() {
        ...
        let stars_in_range = catalog.filter(|star| star.mag >= *min && star.mag < *max).len();
    ...
    for (i, (min, max, _)) in dec_bands.iter().enumerate() {
        ...
        let stars_in_band = catalog.filter(|star| star.dec >= *min && star.dec < *max).len();
    ...
    if let Some(sirius) = catalog.get_star(32349) {
        ...
        if let Some(parallax) = sirius.parallax {
            ...
            println!("    Distance: {:.2} parsecs ({:.2} light years)", 
                     distance_pc, distance_pc * 3.26156);
        }
    } else {
    ...
    if let Some(betelgeuse) = catalog.get_star(27989) {
        ...
        println!("    Position: RA {:.4}°, Dec {:.4}°", betelgeuse.ra, betelgeuse.dec);
    ...
    stars_vec.sort_by(|a, b| {
        let a_pm = a.pm_ra.unwrap_or(0.0).abs().hypot(a.pm_dec.unwrap_or(0.0).abs());
        let b_pm = b.pm_ra.unwrap_or(0.0).abs().hypot(b.pm_dec.unwrap_or(0.0).abs());
    ...
    for (i, star) in stars_vec.iter().take(5).enumerate() {
        ...
        println!("  {}. HIP {} - Proper Motion: {:.2} mas/year", 
                 i + 1, star.hip, pm);
    }
    ...
    // Get terminal width and use it for the grid (with some margins)
    let width: usize = match term_size::dimensions() {
        Some((width, _)) => {
            // Use terminal width minus some margin for borders and indentation
            let available_width = width.saturating_sub(6); // Subtract 6 for margins and borders
            // Ensure minimum width and clamp to terminal size
            available_width.max(40).min(100)
        },
        None => 60, // Default fallback if terminal size detection fails
    };
    ...
    println!("  Map width: {} characters (auto-adjusted to terminal size)", width);
