...
impl Loader {
    ...
    pub fn load_hipparcos_catalog(&self, magnitude_limit: f64) -> Result<catalogs::HipparcosCatalog> {
    ...
    pub fn load_gaia_catalog_from_file<P: AsRef<Path>>(&self, path: P, magnitude_limit: f64) -> Result<catalogs::GaiaCatalog> {
    ...
    pub fn load_gaia_catalog(&self, magnitude_limit: f64) -> Result<catalogs::GaiaCatalog> {
        ...
        println!("Successfully loaded Gaia catalog with {} stars", catalog.len());
        Ok(catalog)
    }
    ...
    pub fn load_ephemeris(&self) -> Result<planetlib::Ephemeris> {
        ...
        unimplemented!("Ephemeris loading not yet implemented")
    }
}
...
mod tests {
    ...
    fn test_synthetic_hip_catalog() {
        ...
        let catalog = loader.load_hipparcos_catalog(6.0).expect("Failed to load synthetic catalog");
        
        // Print the number of stars
        let star_count = catalog.len();
        println!("Loaded {} stars from synthetic Hipparcos catalog", star_count);
        ...
        let orion_belt_stars = catalog.filter(|star| {
            (star.ra >= 80.0 && star.ra <= 85.0) && 
            (star.dec >= -2.0 && star.dec <= 0.0)
        });
        
        println!("Found {} stars in Orion's belt region", orion_belt_stars.len());
        assert!(!orion_belt_stars.is_empty(), "No stars found in Orion's belt region");
    }
