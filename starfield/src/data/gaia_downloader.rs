...
pub fn get_gaia_cache_dir() -> PathBuf {
    ...
    PathBuf::from(home).join(".cache").join("starfield").join("gaia")
}
...
fn download_file<P: AsRef<Path>>(url: &str, path: P) -> Result<()> {
    ...
    loop {
        match response.read(&mut buffer) {
            Ok(0) => break, // EOF
            Ok(n) => {
                file.write_all(&buffer[..n]).map_err(StarfieldError::IoError)?;
                downloaded += n as u64;
                ...
                if downloaded % (5 * 1024 * 1024) == 0 {
                    ...
                    if total_size > 0 {
                        ...
                        print!("\rDownloaded: {:.1}% ({:.1}MB/{:.1}MB) at {:.1} MB/s", 
                               percentage, 
                               downloaded as f64 / 1024.0 / 1024.0,
                               total_size as f64 / 1024.0 / 1024.0,
                               speed);
                    } else {
                        print!("\rDownloaded: {:.1}MB at {:.1} MB/s", 
                               downloaded as f64 / 1024.0 / 1024.0,
                               speed);
                    }
            ...
            Err(e) => return Err(StarfieldError::DataError(format!("Error downloading file: {}", e))),
        }
    }
    ...
    let elapsed = start_time.elapsed().as_secs_f64();
    let speed = if elapsed > 0.0 { downloaded as f64 / elapsed / 1024.0 / 1024.0 } else { 0.0 };
    println!("\rDownload complete: {:.1}MB at {:.1} MB/s in {:.1}s", 
             downloaded as f64 / 1024.0 / 1024.0,
             speed,
             elapsed);
...
fn list_gaia_files() -> Result<Vec<String>> {
    ...
    let response = client.get(GAIA_DR1_BASE_URL)
        .send()
        .map_err(|e| StarfieldError::DataError(format!("Failed to fetch Gaia index: {}", e)))?;
    
    if !response.status().is_success() {
        return Err(StarfieldError::DataError(
            format!("Failed to fetch Gaia index, status: {}", response.status())
        ));
    }
    ...
    let files = re.captures_iter(&html)
    ...
    if files.is_empty() {
        ...
        let fallback_files = (0..=999).map(|i| format!("GaiaSource_000-000-{:03}.csv.gz", i)).collect::<Vec<_>>();
...
pub fn download_gaia_file(filename: &str) -> Result<PathBuf> {
    ...
    if let Some(expected_md5) = checksums.get(filename) {
        if !verify_file(&gz_path, expected_md5)? {
            return Err(StarfieldError::DataError(
                format!("MD5 checksum verification failed for {}", filename)
            ));
        }
    } else {
...
pub fn download_gaia_catalog(max_files: Option<usize>) -> Result<Vec<PathBuf>> {
    ...
    for (i, filename) in files_to_download.iter().enumerate() {
        println!("[{}/{}] Processing {}", i + 1, files_to_download.len(), filename);
    ...
    println!("Downloaded and verified {} Gaia catalog files", downloaded_files.len());
    Ok(downloaded_files)
}
...
mod tests {
    ...
    fn test_cache_dir() {
        let cache_dir = get_gaia_cache_dir();
        assert!(cache_dir.to_str().unwrap().contains(".cache/starfield/gaia"));
    }
}