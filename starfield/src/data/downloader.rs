...
fn decompress_gzip<P: AsRef<Path>, Q: AsRef<Path>>(gz_path: P, output_path: Q) -> Result<()> {
    ...
    let mut header = [0u8; 2];
    {
        ...
        if file_clone.read_exact(&mut header).is_err() || header != [0x1F, 0x8B] {
            return Err(StarfieldError::DataError(
                format!("Invalid gzip file: {:?} is not a valid gzip header", header)
            ));
        }
    }
    ...
    match io::copy(&mut decoder, &mut writer) {
        ...
        Err(e) => {
            ...
            Err(StarfieldError::DataError(format!("Failed to decompress file: {}", e)))
        }
    }
}
...
pub fn download_hipparcos() -> Result<PathBuf> {
    ...
    if !file_exists_and_not_empty(&synthetic_path) {
        ...
        let mut file = BufWriter::new(File::create(&synthetic_path).map_err(StarfieldError::IoError)?);
        ...
        for star in bright_stars.iter() {
            ...
            file.write_all(line.as_bytes()).map_err(StarfieldError::IoError)?;
        }
        ...
        for i in 1..5000 {
            ...
            let mag = 1.0 + (rng.gen_range(0.0..5.0) as f64).powf(1.5);
        ...
        file.flush().map_err(StarfieldError::IoError)?;
        println!("Created synthetic catalog with {} stars", bright_stars.len() + 4999);
    }
