...
fn main() {
    ...
    println!("\nDetected objects:");
    println!("{:3} | {:6} | {:6} | {:5} | {:5}", "ID", "X", "Y", "Width", "Height");
    println!("-----|--------|--------|-------|-------");
    
    for (i, bbox) in bboxes.iter().enumerate() {
        println!(
            "{:3} | {:6} | {:6} | {:5} | {:5}",
            i + 1, bbox.x_min, bbox.y_min, bbox.width, bbox.height
        );
    }
    ...
    println!("F1 score: {:.2}", 2.0 * precision * recall / (precision + recall));
}

/// Create a simulated star field with gaussian stars
fn create_star_field(width: usize, height: usize, num_stars: usize, noise_level: f64) -> Array2<f64> {
    use rand::Rng;
