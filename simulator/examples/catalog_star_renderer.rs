//! Catalog-based star field renderer for telescope simulations
//!
//! This utility extends the enhanced_star_renderer by loading real star data
//! from a catalog (like Hipparcos or Gaia) and rendering photorealistic images
//! of the star field as seen through a specific telescope and sensor.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Default values
    let mut ra_deg = 180.0; // Default to north celestial pole area
    let mut dec_deg = 89.5;
    let mut fov_deg = 2.0;
    let mut catalog_path = "../bright_stars.bin".to_string();
    let mut exposure_seconds = 1.0;
    let mut output_path = "test_output/catalog_star_field.png";
    let mut field_expansion_factor = 2.0; // Factor to expand beyond sensor FOV
    let mut max_magnitude = 8.0; // Maximum (dimmest) star magnitude to include
    ...
    println!("Catalog Star Field Renderer");
    println!("==========================");
    println!("RA: {:.4}°", ra_deg);
    ...
    // Render the star field from catalog data
...
fn draw_svg_overlay(
    ...
    svg_data.push_str(&format!(
        r#"<text x="{}" y="{}" font-family="monospace" font-size="14" fill="magenta">Sensor</text>"#,
...
fn render_catalog_star_field<T: StarPosition + 'static + Clone>(
    ...
    let visible_stars =
        star_projection::filter_stars_in_field(&stars_vec, ra_deg, dec_deg, visualization_fov_deg);
