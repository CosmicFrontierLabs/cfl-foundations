...
fn main() {
    ...
    println!("Test image saved to: {}", output_path.display());

    // Try to save the SVG directly for comparison
    let svg_path = output_dir.join("test_svg.svg");
    std::fs::write(&svg_path, svg_data).expect("Failed to save SVG file");
    println!("SVG file saved to: {}", svg_path.display());
}
