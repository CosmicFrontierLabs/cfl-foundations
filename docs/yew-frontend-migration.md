...
This document explains how the meter-sim test-bench frontends were migrated from plain HTML/JavaScript to Yew-based WebAssembly applications.

## Overview

The `calibrate_serve` and `fgs_server` servers originally used inline HTML templates with embedded JavaScript. This migration converts them to modern Yew (Rust WebAssembly) frontends while maintaining:
...
fgs_server.rs
  ├─> Serves WASM bundle via /static/
  └─> WebSocket endpoints for live status/image streaming

test-bench-frontend/
  ├─> fgs_app.rs (Yew component)
  ├─> fgs_main.rs (WASM entry point)
  └─> dist/fgs/ (trunk build output)
       ├─> fgs_wasm.js
       ├─> fgs_wasm_bg.wasm
       └─> index.html
```
...
Frontend communicates with backend via HTTP and WebSocket.
