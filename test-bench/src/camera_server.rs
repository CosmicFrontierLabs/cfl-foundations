...
type SharedFrame = Arc<(Array2<u16>, FrameMetadata)>;
    ...
    while let Ok(shared) = rx.recv() {
        ...
        if tracking_enabled {
            ...
            if fgs.is_none() {
                ...
                let export_settings = tracking_shared.export_settings.blocking_read().clone();

                let csv_writer: Option<Arc<std::sync::Mutex<csv::Writer<std::fs::File>>>> =
                    if export_settings.csv_enabled {
                        match std::fs::File::create(&export_settings.csv_filename) {
                            ...
                            Err(e) => {
                                ...
                                let mut err = tracking_shared.export_last_error.blocking_write();
                    ...
                    match event {
                        ...
                        FgsCallbackEvent::FrameProcessed {
                            ...
                            if let Some((ref writer, ref export_dir)) = frame_writer_for_cb {
                                let png_path =
                                    export_dir.join(format!("frame_{frame_number:06}.png"));
                                ...
                                let json_path =
                                    export_dir.join(format!("frame_{frame_number:06}.json"));
                                ...
                                if writer.write_frame_nonblocking(
                                    frame_data.as_ref(),
                                    png_path,
                                    FrameFormat::Png,
                                ) {
                                    tracking_cb
                                        .export_frames_written
                                        .fetch_add(1, Ordering::SeqCst);
                                } else {
                                    tracing::debug!(
                                        "Frame export queue full, dropping frame {}",
                                        frame_number
                                    );
                                }
                            }
...
fn encoding_worker(
    ...
    let mut last_publish = std::time::Instant::now() - MJPEG_MIN_INTERVAL;
